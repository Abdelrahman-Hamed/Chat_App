...
//            Registry reg = LocateRegistry.getRegistry("10.145.4.235", 5001, new SslRMIClientSocketFactory());
            Registry reg = LocateRegistry.getRegistry("192.168.43.80", 5001);
...
//        reg = LocateRegistry.getRegistry("10.145.1.152", 5001);
        reg = LocateRegistry.getRegistry("192.168.43.80", 5001);
...
//        } catch (RemoteException | NotBoundException e) {
    ...
    @Override
    public void removeNotification(int fromUserId) throws RemoteException {
    ...
    public void addMeAsFriend(int newFriendId) {
        try {
            chatService.addMeAsFriend(user.getId(), newFriendId);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
